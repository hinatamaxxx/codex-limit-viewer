...
- **ホバーで詳細確認**: 初期状態はオフです。オンにすると各バケットの詳細残量、リセットまでの時間（日数・時間・分）、翌日以降のリセット日、最終取得時刻がフェード表示され、カーソルが表示領域を離れるとすぐ消え始めます。オフのときはアプリ名のツールチップを中央に表示します。
...
- **ホバー**: 詳細ウィンドウを開きます（初期状態はオフ）。残量表示と詳細画面の両方からカーソルを離すと、待たずにフェードして閉じます。オフではアプリ名のツールチップを中央に表示します。
