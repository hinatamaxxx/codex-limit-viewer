...
> Grok対応は開発中のv0.1.2に含まれます。公開中のv0.1.1にはまだ含まれていません。
...
- **4つのサービス連携**: Codexはデスクトップアプリ同梱または単独のCodex CLI、Antigravityは`agy`、Grokは公式Grok CLIから60秒ごとに取得します。Claude Codeは公式status lineから使用率を受け取ります。本アプリが認証情報を抽出・保持することはありません。
- **Grokの週次利用枠**: 公式Grok Build CLI（`grok.exe`）から残り割合とリセット日時を取得し、右クリックメニューの「タスクバー表示」で上段または下段に表示できます。
