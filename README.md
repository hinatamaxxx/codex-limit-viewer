...
- **ホバーで詳細確認**: 残量表示にカーソルを合わせると、各バケットの詳細残量、リセットまでの時間（日数・時間・分）、最終取得時刻がフェード表示されます。右クリックメニューでオフにすると、代わりに標準のツールチップでアプリ名を表示します。
...
- **ホバー**: 詳細ウィンドウを開きます（初期状態はオン）。残量表示と詳細画面の両方からカーソルを離すとフェードして閉じます。オフにするとアプリ名のツールチップを表示します。
